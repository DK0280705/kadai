...
// With this, it does not matter whether if the width is a factor of 4 or not.
...
void bitmap_fill_half(bitmap_t* bitmap, pixel_data_t const pixel)
{
    int width = bitmap->info_header.width;
    int height = bitmap->info_header.height;

    for (int y = 0; y < height; ++y) {
        // Might not perfectly half if the width is odd
        for (int x = 0; x < width / 2; ++x) {
            memcpy(bitmap->pixel_data + y * width + x, &pixel, 3);
        }
    }
}
...
{
    if (argc < 5) {
        fprintf(stderr, "USAGE: %s <FILENAME:path> <B:int> <G:int> <R:int>\n", argv[0]);
        exit(-1);
    }
